...
红外图像能够在复杂环境和弱光条件下提供稳定的热目标信息，可见光图像则保留丰富的纹理与场景结构。TPTAF 将检测语义作为任务先验，引导表示层面的跨模态特征交互。解耦编码器（DE）将每个模态组织到结构空间和判别空间；检测相关语义进一步生成任务先验；三元注意力模块（TAM）协调结构约束、判别细节和检测先验信息，实现任务引导的跨模态特征交互。联合训练过程中，不确定性加权学习（UWL）用于平衡融合目标与检测目标。
